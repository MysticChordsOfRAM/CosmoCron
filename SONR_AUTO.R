...
###############################################################################\
##                                                                            ##
##   #####    #####   ##   ##  ######       Summary of National Reporting     ##
##  ##   ##  ##   ##  ###  ##  ##   ##      -----------------------------     ##
##  #        ##   ##  ## # ##  ##   ##      Automated Data Warehouse Job      ##
##   #####   ##   ##  ##  ###  ######       Freq: Weekly (Monday 04:00)       ##
##       ##  ##   ##  ##   ##  ##   ##                                        ##
##  ##   ##  ##   ##  ##   ##  ##   ##      STATUS: ACTIVE                    ##
##   #####    #####   ##   ##  ##   ##      OWNER:  DEVLIN                    ##
##                                                                            ##
...
stringify_date <- function(date_object, dash = FALSE) {
  ...
  if (dash) {
    str_date <- date_object %>% format("%Y-%m-%d")
  } else {
    str_date <- date_object %>% format("%Y%m%d")
  }
...
table_update_better <- function(db_con, table_name, updater) {
  ...
  if (length(periods_to_replace) > 0) {

     delete_qry <- glue_sql(
    "DELETE FROM {`table_name`} WHERE time_period IN ({periods_to_replace*})",
    .con = db_con
    )

    dbExecute(db_con, delete_qry)
...
fetch_fred <- function(var_id) {

  print(var_id)
  url_series <- str_c("series_id=", var_id)
  api_url <- str_c(url_base, url_series, url_key)
